            ...
            try {
                val posts = getPosts(client)
                    .map { post ->
                        ...
                        var listCommentWithAuthor: List<CommentWithAuthor> = emptyList()
                        async {
                            author = getAuthor(client, post.authorId)
                        }.await()
                        async {
                            listCommentWithAuthor = getComments(client, post.id).map {
                                async {
                                    CommentWithAuthor(it, getAuthor(client,it.authorId))
                                }
                            }.awaitAll()
                        }.await()
                        PostWithCommentsAndAuthor(post, listCommentWithAuthor, author!!)
                    }
